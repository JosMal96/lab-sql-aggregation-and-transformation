...
-- 1. You need to use SQL built-in functions to gain insights relating to the duration of movies:
   ...
   SELECT MIN(length) AS 'Shortest Movie' , MAX(length) AS 'Longest Movie' FROM film;
   -- 1.2. Express the average movie duration in hours and minutes. Don't use decimals.
   SELECT CONCAT(FLOOR(AVG(length) / 60), ' hours and ', MOD(FLOOR(AVG(length)), 60), ' minutes') AS 'Average Movie Duration' FROM film;
...
--         Hint: To do this, use the rental table, and the DATEDIFF() function to subtract the earliest date in the rental_date column from the latest date.
	   SELECT DATEDIFF(MAX(rental_date), MIN(rental_date)) AS 'Days that the company has been operating' from rental;
...
SELECT IFNULL(title, 'N/A') AS 'Title', IFNULL(rental_duration, 'N/A') AS 'Rental Duration'
FROM film
ORDER BY title ASC;
...
SELECT rating, round(avg(length)) AS 'Average Duration'
	FROM film
	GROUP BY rating
